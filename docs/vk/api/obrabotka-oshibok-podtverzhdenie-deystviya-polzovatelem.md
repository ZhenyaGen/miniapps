...
**Раздел:** Использование API → Обработка ошибок → Подтверждение действия пользователем  
...
Приложение должно показать стандартное окно подтверждения с текстом из

`confirmation_text` и двумя кнопками Продолжить и Отмена. В случае подтверждения действия пользователем, нужно повторить запрос с дополнительным параметром `confirm = 1`.
