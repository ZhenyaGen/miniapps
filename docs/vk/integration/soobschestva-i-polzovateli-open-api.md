...
**Раздел:** Интеграция → Сообщества и пользователи → Open API  
...
В случае успеха завершает сессию пользователя внутри платформы Open API. Генерирует событие
...
В случае успеха лишает приложение прав на доступ к данным пользователя. Генерирует событие
...
Возвращает сессионные данные пользователя. Этот метод нужно использовать для получения данных сессии, когда вы точно знаете, что пользователь авторизован ВКонтакте и в приложении, чтобы избежать задержек при асинхронном вызове `VK.Auth.getLoginStatus`.
...
Для того, чтобы вы могли проверить, что пользователь действительно авторизовался через платформу Open API, в сессионных данных передается параметр `sig`. Он служит цифровой подписью данных. После авторизации сессионные данные записываются в cookie с именем

`vk_app_<APP_ID>`, где `<APP_ID>` - это идентификатор вашего приложения. Данные записаны в виде пар `parameter_name` = `parameter_value`, соединенных разделительным символом `&`:

```
expire=1271238742&mid=100172&secret=97c1e8933e&sid=549b550f608e4a4d247734941de
bb5e68df50a66c58dc6ee2a4f60a2&sig=372df9795fe8dd29684a2f996872457c
```
...
```
expire=1271238742mid=100172secret=97c1e8933esid=549b550f608e4a4d247734941debb5
e68df50a66c58dc6ee2a4f60a26FF1PUlZfEyutJxctvtd
```
...
- `method` ( `string`) — название [метода API;](https://dev.vk.ru/ru/method)
- `params` ( `object`) — параметры метода (параметр версии API `v` обязателен);
- `callback` ( `function`) — функция обратного вызова.

Вызов метода [`users.get`](https://dev.vk.ru/ru/users.get) через Open API:
...
- `VK.Widgets.ContactUs` — [виджет «Напишите нам»](https://dev.vk.ru/ru/widgets/contact-us).
- `VK.Widgets.Comments` — [виджет комментариев](https://dev.vk.ru/ru/widgets/comments).
- `VK.Widgets.Post` — [виджет «Запись на стене»](https://dev.vk.ru/ru/widgets/post).
- `VK.Widgets.Group` — [виджет сообщества](https://dev.vk.ru/ru/widgets/group).
- `VK.Widgets.Like` — [виджет «Мне нравится»](https://dev.vk.ru/ru/widgets/like).
- `VK.Widgets.Recommended` — [виджет рекомендаций](https://dev.vk.ru/ru/widgets/recommended).
- `VK.Widgets.Poll` — [виджет опроса](https://dev.vk.ru/ru/widgets/poll).
- `VK.Widgets.Subscribe` — [виджет «Подписаться на автора»](https://dev.vk.ru/ru/widgets/subscribe).

## Обработка событий

Для обработки событий в Open API используйте методы `VK.Observer.subscribe` и
...
| Событие | Описание |
|---|---|
| `auth.login` | Возникает при авторизации пользователя с помощью метода `VK.Auth.login`. |
| `auth.logout` | Возникает при выходе пользователя с помощью метода |
...
`widgets.comments.new_comment` Добавлен новый комментарий в [виджете комментариев](https://dev.vk.ru/ru/widgets/comments).

`widgets.comments.delete_comment` Удалён комментарий из виджета комментариев.

`widgets.groups.joined` Новое вступление в [виджете сообщества](https://dev.vk.ru/ru/widgets/group).

`widgets.groups.leaved` Выход участника в виджете сообщества.

`widgets.like.liked` Новая отметка Мне нравится в [виджете](https://dev.vk.ru/ru/widgets/like).
...
`widgets.subscribed` Новый подписчик в [виджете](https://dev.vk.ru/ru/widgets/subscribe) «Подписаться на автора».

| Событие | Описание |
|---|---|
| `widgets.unsubscribed` | Удалившийся подписчик в виджете «Подписаться на автора». |

Пример использования `VK.Observer` с [виджетом «Мне нравится»](https://dev.vk.ru/ru/widgets/like):
...
Используйте метод `VK.Retargeting.Init`, чтобы инициализировать пиксель. В качестве единственного параметра метод принимает код пикселя, который вы можете получить в настройках рекламного кабинета или методом [`ads.createTargetPixel`](https://dev.vk.ru/ru/ads.createTargetPixel) [.](https://dev.vk.ru/ru/ads.createTargetPixel)
