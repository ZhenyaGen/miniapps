...
**Раздел:** Интеграция → Виджеты → Публикация ссылок  
...
```html
<script type="text/javascript" src="https://vk.ru/js/api/share.js?93"
...
Если содержимое, в том числе включающее кнопки публикации ссылок ВКонтакте, обновляется на вашем сайте динамически, то вы можете использовать текст, возвращаемый функцией

`VK.Share.button` для создания кнопок публикации «на лету». Всё что вам необходимо для этого сделать, это выделить в нужном месте контейнер для кнопки и после загрузки содержимого выполнить код, подобный следующему:

```js
document.getElementById('vk_share_button').innerHTML =
VK.Share.button('http://mysite.com', {type: 'link'});
```
...
[https://vk.ru/share.php?url={адрес_страницы}](https://vk.ru/share.php?url=%7B%D0%B0%D0%B4%D1%80%D0%B5%D1%81_%D1%81%D1%82%D1%80%D0%B0%D0%BD%D0%B8%D1%86%D1%8B%7D)

Код для ее размещения может выглядеть так:

```html
<a href="https://vk.ru/share.php?url=http://mysite.com"
target="_blank">Поделиться ВКонтакте</a>
```
...
- Заголовок. Если на публикуемой странице есть теги `og:title`, `mrc__share_title` или

`twitter:title`, то заголовок будет взят из одного из них (указанного первым), иначе будет использован тег `<title>`.
...
Важно! Данные для публикации могут кэшироваться. Если вы изменили данные на своем сайте, а ВКонтакте при публикации отображаются старые значения, сбросьте кэш для ссылки на странице

[`pages.clearCache`](https://dev.vk.ru/ru/method/pages.clearCache) [.](https://dev.vk.ru/ru/method/pages.clearCache)
...
```js
document.write(VK.Share.button({url: 'http://mysite.com', title: 'Заголовок
страницы'}, {type: 'custom', text: '<img src="http://vk.ru/images/vk32.png"
/>'}));
```
...
[Сохранить](https://vk.com/share.php?url=https%3A%2F%2Fdev.vk.ru%2Fru%2Fwidgets%2Fshare)
