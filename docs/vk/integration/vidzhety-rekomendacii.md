...
**Раздел:** Интеграция → Виджеты → Рекомендации  
...
Популярность материалов определяется с помощью виджета [Нравится](https://dev.vk.ru/ru/widgets/like), который необходимо установить перед использованием виджета для рекомендаций.
...
- `verb` ( `integer`, [ `0,1` ]) — вариант формулировки текста внутри блока. Возможные значения: `1`
— «Интересно», `0` — «Нравится». Значение по умолчанию: `0`.
- `sort` ( `string`) — способ сортировки материалов. Возможные значения:

- `friend_likes` — по количеству друзей, оставивших отметки «Нравится»;
- `likes` — по общему количеству отметок. Значение по умолчанию: `friend_likes`.
...
- `parent` — открывать во фрейме, который содержит виджет. Значение по умолчанию: `parent`.
...
window.onload = function () {
  ...
  VK.Widgets.Recommended('vk_recommend', {limit: 4, max: 10, period: 'month',
verb: 1});
}
</script>
```
...
```
<!-- Put this script tag to the place, where the Recommended block will be
-->
