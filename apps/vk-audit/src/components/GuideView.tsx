...
const STEPS: Step[] = [
  {
    ...
    points: [
      ...
      ['Забрать файлом', 'внизу отчёта: бриф в TXT, таблица в CSV (каждая строка — запись, клип, видео или снимок одними столбцами: сразу строится сводная) и PDF через печать браузера'],
    ],
  },
  {
    id: 'summary',
  ...
  },
  {
    id: 'clips',
    tab: 'Клипы',
    title: 'Клипы считаются отдельно',
    screen: '7-klipy.png',
    lead: 'Клип живёт не в ленте сообщества, а в ленте клипов: охват там '
      + 'почти не зависит от числа подписчиков, зато сильно — от первых секунд. '
      + 'Собирается по кнопке, это отдельные запросы поверх отчёта.',
    points: [
      ['Выстрелы', 'клипы, набравшие больше трёх медиан, и их доля во всех просмотрах. Обычно два-три клипа приносят больше половины — разбирать надо именно их'],
      ['Разброс', 'во сколько раз лучший клип обошёл серединный. Пять и больше — норма для этой ленты, средний уровень тут не значит ничего'],
      ['Длина', 'узкими корзинами: до 15 секунд, 15–30, 30–60. У клипов разница между 15 и 40 секундами больше, чем у видео между тремя минутами и пятью'],
      ['Как менялось', 'медиана просмотров по месяцам — видно, лента к вам разогревается или остывает'],
      ['Мимо стены', 'клипы, не выложенные записью. Подписчики сообщества их не видят — только те, кому клип покажет лента'],
    ],
  },
  {
    id: 'video',
    ...
    screen: '8-roliki.png',
    lead: 'Обычные ролики из раздела «Видео» — без клипов, они на соседней '
      + 'вкладке. Плюс все комментарии: со стены и из-под роликов. '
      + 'Собирается той же кнопкой, что и клипы.',
    points: [
      ['Два счётчика просмотров', 'у записи и у ролика внутри — они расходятся в разы. Вкладка показывает оба и говорит, во сколько раз. Если ролики смотрят чаще, разбор по стене занижает охват'],
      ['Против клипов', 'медианы обычных видео и клипов рядом — видно, какая лента приносит охват. В остальные цифры вкладки клипы не входят: у них своя'],
      ['Длительность', 'медиана просмотров по длине ролика: видно, какой формат досматривают'],
      ['Комментарии из двух мест', 'со стены и из-под роликов. У клипа своё обсуждение, и клип мимо стены живёт только там'],
      ['Вопросы без ответа', 'комментарии со знаком вопроса, где автор не ответил. Самая дешёвая вовлечённость — человек уже написал. Клик ведёт прямо к ролику'],
      ['О чём пишут', 'частые слова из комментариев людей, без ответов автора. Повторяющееся слово — готовая тема для поста'],
      ['Только своё', 'чужие ролики, добавленные к себе в раздел «Видео», в разбор не идут — иначе медианы и «лучшие видео» уезжают в чужие цифры. Сколько их было, вкладка скажет отдельно'],
    ],
  },
  {
    id: 'photos',
    tab: 'Фото',
    title: 'Фотографии считаются отдельно',
    screen: '9-foto.png',
    lead: 'У снимка свои лайки и свои комментарии — с реакциями записи '
      + 'они не совпадают. И живёт он дольше: картинка из альбома собирает '
      + 'реакции месяцами, когда запись уже ушла из ленты.',
    points: [
      ['Лайки снимка против лайков записи', 'главная цифра вкладки. Если реагируют на картинку, текст можно сокращать, а кадр выбирать тщательнее. Если на запись — картинка тут иллюстрация, вкладываться в неё незачем'],
      ['Мимо ленты', 'снимки, которых не было в записях. Если они собрали заметную долю лайков — спрос есть, а записи под него нет: выложите лучшие'],
      ['Форма кадра', 'вертикальные, квадратные, горизонтальные. В ленте на телефоне это разная площадь экрана, и разница копится'],
      ['Как менялось', 'медиана лайков на снимок по месяцам'],
    ],
  },
  {
    id: 'rivals',
