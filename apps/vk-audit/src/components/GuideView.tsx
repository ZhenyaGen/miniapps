...
const STEPS: Step[] = [
  ...
  },
  {
    id: 'ai',
    tab: 'Нейросеть',
    title: 'Разбор нейросетью за три шага',
    screen: '7-ii.png',
    lead: 'Приложение считает цифры, но не пишет текстов. Нейросеть наоборот: '
      + 'пишет хорошо, а считать не умеет и склонна выдумывать. Поэтому цифры '
      + 'ей отдаются готовыми — вместе с прямым запретом их менять.',
    points: [
      ['Шаг 1 — «Скопировать бриф»', 'весь отчёт текстом уедет в буфер обмена: метрики, форматы, зоны роста, «чего на странице нет», план на четыре недели и тексты лучших и слабых постов'],
      ['Шаг 2 — «Открыть DeepSeek»', 'откроется чат в браузере. Подойдёт любая нейросеть, которая принимает длинный текст, — DeepSeek просто бесплатный и без регистрации по номеру'],
      ['Шаг 3 — вставить и отправить', 'задание уже вписано в начало брифа: что разобрать, какие диаграммы построить, что предложить. Дописывать ничего не нужно'],
      ['«Что именно скопируется»', 'раскрывающийся список: семь разделов, всё из этого отчёта. Ключ доступа и личные данные в бриф не попадают'],
      ['«Что вернёт нейросеть»', 'пример ответа на вымышленных данных — чтобы заранее понимать, что придёт текст с выводами и планом, а не таблица'],
      ['Дальше можно разговаривать', 'попросить переписать слабый пост, предложить темы на месяц или нарисовать графики по рядам из конца брифа'],
    ],
  },
  {
    id: 'rivals',
