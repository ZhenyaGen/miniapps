...
const STEPS: Step[] = [
  {
    ...
    points: [
      ...
      ['«Проверить мою страницу»', 'разбирает вашу без ввода ссылки — внутри ВКонтакте приложение узнаёт вас по параметрам запуска'],
      ['Период', 'месяц, три месяца, полгода или год. Полгода — рабочий срез; месяц берите только чтобы быстро глянуть, на десятке постов медианы это шум'],
      ['Доступ к данным', 'отдельного входа нет. Разрешение на чтение записей ВКонтакте спросит один раз, когда нажмёте «собрать» — тогда же видно, зачем оно'],
      ['«Показать мои сообщества»', 'в режиме «Сообщество» — подтягивает те, где вы администратор, редактор или модератор. Дальше выбор одним нажатием, без копирования ссылки'],
      ['«Открыть демо-отчёт»', 'все экраны на вымышленном сообществе, вообще без обращений к ВКонтакте'],
      ['«Скопировать бриф»', 'внизу отчёта — весь разбор текстом: метрики, форматы, зоны роста, план и готовые ряды под диаграммы. Вставляется в чат с любой нейросетью'],
    ],
  },
  {
    id: 'summary',
  ...
  },
  {
    ...
    points: [
      ...
      ['Размер важнее всего', 'берутся только сопоставимые — от трети до трёх ваших размеров. Сравнение с миллионником красит всё в красный и не говорит ничего'],
      ...
      ['Профили тоже сравниваются', 'приложение предупредит, если в одном сравнении окажутся вперемешку страницы и сообщества: ВК ранжирует их по-разному'],
    ],
  },
];
